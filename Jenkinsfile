                ...
                script {
                    for (int i=0; i<10; i++) {
                        try {
                            ...
                            do
                                IMAGE_NAME=${library#*/}
                                docker --config /home/metersphere/.docker buildx build --build-arg MS_VERSION=MS-1.0 -t ${IMAGE_PREFIX}/${IMAGE_NAME}:\${TAG_NAME:-\$BRANCH_NAME} --platform linux/amd64,linux/arm64 ./$library --push
                            done
                            '''
                            break
                        } catch (Exception e) {
